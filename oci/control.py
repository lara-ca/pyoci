...
def design(u, y, Td, Cf, model, model_params, L):
    ...
    if flag_1 == 0 and flag_2 == 0:
        ...
        if model == 'OE':
            ...
            model = ps.oe( params['nb'],  params['nf'], params['nk'], u_til, yf) 
            ...
            model = ps.arx(params['na'], params['nb'], params['nk'], u_til, yf)
            # identify C_til = 1/C_i 
            C_til = [[signal.TransferFunction(
                  oci.clean_coefficients(model.B[i][j]),
            ...
            model = ps.armax(params['na'], params['nb'], params['nc'], params['nk'], u_til, yf)
            # identify C_til = 1/C_i 
            C_til = [[signal.TransferFunction(
                  oci.clean_coefficients(model.B[i][j]),
                  oci.clean_coefficients(model.A[i][j]), dt=1)
            ...
            model = ps.bj(params['nb'], params['nc'], params['nd'], params['nf'], params['nk'], u_til, yf)
            # identify C_til = 1/C_i
            C_til = [[signal.TransferFunction(
                  oci.clean_coefficients(model.B[i][j]),
                  oci.clean_coefficients(model.F[i][j]), dt=1)
        ...
        if flag_3 == 2:
          ...
          print(
           "The inverse of (C_til) is unstable. It is not recommended to proceed with the OCI method."
          )
